<cfoutput>
    ...
    <cfquery name="leave_requests">
        select a.*, l.leave_title, concat(emp.first_name,' ',emp.middle_name,' ',emp.last_name) as 
        name
    ...
    <cfquery name="get_approved_requests">
        select a.*, l.leave_title, emp.official_email, concat(emp.first_name,' ',emp.middle_name,' 
    ...
    <cfif structKeyExists(form, 'Approve') or structKeyExists(form, 'Reject') or structKeyExists(form, 'Partial_leave')>
        ...
        <cfquery name="get_working_days">
            select b.* , a.workingdays_group, a.official_email from employee a, working_days b where a.employee_id = '#get_leave_detail.employee_id#' 
            ...
            <cfif evaluate("get_working_days.#dayName#") eq '1'>
                ...
                <cfelseif isdefined("form.Reject")>
                    <cfquery name="Approved_leaves">
                        Insert into leaves_approval (leave_id, leave_Date, action, approved_as)
                        values (<cfqueryparam value='#url.id#'>
                        , '#dateFormat('#index#', 'yyyy-mm-dd')#', 'Rejected', '0')
                    </cfquery>
                <cfelseif isDefined("form.Partial_leave")>
                    ...
                    <cfelseif evaluate('form.date#counter1#') eq 0.5>
                        <cfquery name="Approved_leaves">
                            Insert into leaves_approval (leave_id, leave_Date, action, approved_as)
                            values (<cfqueryparam value='#url.id#'>
                            , '#dateFormat('#index#', 'yyyy-mm-dd')#', 'Approved', '0.5')
                        </cfquery>
                    ...
                    <cfif evaluate('form.date#counter1#') eq 1 or evaluate('form.date#counter1#') 
                          eq 0.5>
                        <cfquery name="update_leave_balance">
                            Update employee_leaves
                            Set
    ...
    <cfif structKeyExists(url, 'request_id')><!--- Front end when admin user want to approve or 
                                             reject a leave--->
        <cfquery name="get_leave_detail">
        ...
        <cfquery name="get_working_days">
            select b.* , a.workingdays_group from employee a, working_days b where a.employee_id = '#get_leave_detail.employee_id#
            ' and b.group_id = a.workingdays_group
        </cfquery>
        <!---            <cfdump  var="#employee_detail#"> <cfabort> --->
        <!--- ___________________________________________ Front End 
        ...
        <div class="employee_box">
            ...
            <div class="row">
                <div class="col-md-3">
                    From Date: 
                    <p>
                    #dateFormat(get_leave_detail.from_date, 'dd-mmm-yyyy')#
                    <p>
                </div>
                <div class="col-md-3">
                    To Date: 
                    <p>
                    #dateFormat(get_leave_detail.to_date, 'dd-mmm-yyyy')#
                    <p>
                </div>
                <div class="col-md-3">
                    Days:
                    <p>
                        #get_leave_detail.leave_days#
                    </p>
                </div>
                <div class="col-md-3">
                    Leave Title:
                    <p>
                        #leave_requests.leave_title#
                    </p>
                </div>
            </div>
            <div class="row">
            ...
            <form action="leave_approval.cfm?id=#url.request_id#" method="post">
                <div id="leaveSelection" style="display:none;">
                    ...
                    <div class="text-center mb-2">
                        <p style="font-weight: 600;">
                            Select Payment Type For Partial Leave Approval According Dates
                        </p>
                    </div>
                    <div class="row mb-2">
                        <cfset counter = 0>
                        <cfloop index="index" from="#get_leave_detail.from_date#" 
                                to="#get_leave_detail.to_date#">
                            <cfset counter += 1>
                            <cfset day = dayOfWeek(index)>
                            <cfset dayName = dayOfWeekAsString('#day#')>
                            <cfif evaluate("get_working_days.#dayName#") eq '1'>
                                <div class="col-md-6 mb-2 mt-2">
                                    ...
                                    style="border-radius: 0;" id="#DateFormat("#index#", 
                                    "yyyy-mm-dd")#" name = "#DateFormat("#index#", "yyyy-mmm-dd")#" 
                                    required> 
                                                                            </div>--->
                                    <span class="form-check-label ml-4 mt-1">
                                        #DateFormat("#index#", "yyyy-mmm-dd")# 
                                        ( 
                                        #dayName# 
                                        )
                                    </span>
                                ...
                                <div class="col-md-4 mb-2 ml-4">
                                    <select class="form-select" name="date#counter#" 
                                            id="Payment_type">
                                        <option value="1">
                                            Full Paid
                                        </option>
                    ...
                    <div class="text-left">
                        <input type="submit" id="Partial_leave" value="Approved Partially" 
                               name="Partial_leave" class="btn btn-outline-danger mb-2 ml-2">
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <textarea class="form-control" id="txt_remarks" name="txt_remarks" 
                                  placeholder="Please Write Some Remarks According to Your Action (Approve or Reject)" 
                                  required>
                        </textarea>
                    </div>
                </div>
                <div class="row mt-3">
                    <div class="d-flex justify-content-end flex-wrap" style="gap: 8px;">
                        ...
                        <button id="" 
                                onclick="document.getElementById('leaveSelection').style.display='inline'; this.disabled=true" 
                                name="Partial" class="btn btn-outline-danger">
                            Approve Partial Leave
                        </button>
                        <input type="submit" id="Approve_leave" value="Approve Leave" name="Approve"
                               class="btn btn-outline-success">
                        <input type="submit" id="Reject_leave" value="Reject Leave" name="Reject"
                    ...
                    <td>
                        <a href="leave_approval.cfm?request_id=#id#">
                            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" 
                                 fill="currentColor" class="bi bi-journal-medical" 
                                 viewbox="0 0 16 16">
